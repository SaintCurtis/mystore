...
const STEPS = [
  ...
  { key: "processing",label: "Processing",        sub: "Verifying and packing your order",     icon: Clock,         activeColor: "text-brand-600 dark:text-brand-400",   activeBg: "bg-brand-100 dark:bg-brand-500/15",   activeBorder: "border-brand-300 dark:border-brand-500/40"   },
  { key: "shipped",   label: "Shipped",           sub: "Your order is on its way",             icon: Truck,         activeColor: "text-brand-600 dark:text-brand-400",     activeBg: "bg-brand-100 dark:bg-brand-500/15",     activeBorder: "border-brand-300 dark:border-brand-500/40"     },
                  ...
                  return (
                    <div key={step.key} className="relative flex items-start gap-4">
                      <div className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-500 ${done ? `${step.activeBg} ${step.activeBorder}` : "bg-zinc-50 dark:bg-[#0d0d0d] border-zinc-200 dark:border-[#2a2a2a]"}`}>
                        ...
                        {current && <span className="absolute -inset-1.5 rounded-full animate-ping opacity-20 bg-brand-400" />}
                      </div>
                      <div className="flex-1 pt-1.5">
                        <div className="flex items-center justify-between gap-2">
                          <p className={`text-sm font-semibold ${done ? "text-zinc-900 dark:text-[#f1f1f1]" : "text-zinc-400 dark:text-[#444]"}`}>
                            {step.label}
                            {current && (
                              <span className="ml-2 inline-flex items-center rounded-full bg-brand-100 dark:bg-brand-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-brand-700 dark:text-brand-400">
                                Current
                              </span>
                            )}
                          </p>
                ...
                {order.items?.map((item) => (
                  <div key={item._key} className="flex gap-4 px-6 py-4">
                    ...
                    <div className="flex flex-1 flex-col justify-between min-w-0">
                      <Link href={`/products/${item.product?.slug}`} className="font-medium text-zinc-900 dark:text-[#f1f1f1] hover:text-brand-600 dark:hover:text-brand-400 transition-colors line-clamp-2">
                        {item.product?.name ?? "Unknown Product"}
                      </Link>
                      <p className="text-sm text-zinc-500 dark:text-[#a3a3a3]">Qty: {item.quantity}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-semibold text-zinc-900 dark:text-brand-400">{formatPrice((item.priceAtPurchase ?? 0) * (item.quantity ?? 1))}</p>
            ...
            <div className="rounded-2xl border border-zinc-200 dark:border-[#1a1a1a] bg-white dark:bg-[#111111] p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="border-t border-zinc-100 dark:border-[#1a1a1a] pt-2 flex justify-between font-bold text-base">
                  ...
                  <span className="text-zinc-900 dark:text-brand-400">{formatPrice(order.total)}</span>
                </div>
              </div>
            </div>
