...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; className: string; icon: React.ReactNode }> = {
    pending:   { label: "Pending",   className: "bg-brand-100 text-brand-800 dark:bg-brand-900/30 dark:text-brand-400",         icon: <Clock className="h-3 w-3" />       },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { label: "Total Products", value: s?.totalProducts ?? 0, icon: Package,       color: "text-brand-600 dark:text-brand-400",      bg: "bg-brand-50 dark:bg-brand-900/20"       },
          ...
          { label: "Total Orders",   value: s?.totalOrders ?? 0,   icon: ShoppingCart,  color: "text-brand-600 dark:text-brand-400",     bg: "bg-brand-50 dark:bg-brand-900/20"     },
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 px-5 py-4">
            ...
            <Link href="/admin/orders" className="flex items-center gap-1 text-xs text-brand-600 dark:text-brand-400 hover:underline">
              ...
              (recentOrders as any[]).map((order: any) => (
                <div key={order._id} className="flex items-center justify-between px-5 py-3">
                  ...
                  <div className="flex items-center gap-3 shrink-0 ml-3">
                    {order.totalAmount && (
                      <span className="text-sm font-semibold text-zinc-900 dark:text-brand-400">
        ...
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
          <div className="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 px-5 py-4">
            ...
            <Link href="/admin/inventory" className="flex items-center gap-1 text-xs text-brand-600 dark:text-brand-400 hover:underline">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <Link
              ...
              className="flex flex-col gap-2 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 transition-all hover:border-brand-500/50 hover:shadow-md dark:hover:border-brand-500/30"
            >
