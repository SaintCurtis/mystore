...
export type Product = {
  ...
  variantGroups?: Array<{
    type?: "processor" | "ram" | "ssd" | "gpu" | "color" | "touchscreen";
    label?: string;
    options?: Array<{
      label?: string;
      hint?: string;
...
export type EXISTING_SUB_QUERY_RESULT = {
  _id: string;
} | null;
...
export type REFERRAL_BY_CODE_QUERY_RESULT = {
  _id: string;
  _type: "referral";
  _createdAt: string;
  _updatedAt: string;
...
// Query: *[  _type == "product"  && slug.current == $slug][0] {  _id,  name,  "slug": slug.current,  description,  price,  "condition": condition->{ _id, title, "slug": slug.current },  "brand": brand->{ _id, title, "slug": slug.current },  "model": model->{ _id, title, "slug": slug.current },  "images": images[]{ _key, asset->{ _id, url }, hotspot },  "videos": videos[]{ _key, asset->{ _id, url, mimeType } },  category->{    _id,    title,    "slug": slug.current,    "parentSlug": parentCategory->slug.current,    "parentTitle": parentCategory->title  },  material,  color,  dimensions,  stock,  featured,  assemblyRequired,  isNegotiable,  // ── Tech Specs ─────────────────────────────────────────────  specDisplay,  specProcessor,  specRAM,  specStorage,  specGPU,  specBattery,  specOS,  specConnectivity,  specRefreshRate,  specWeight,  "specExtras": specExtras[]{ label, value },  // ── Variants ──────────────────────────────────────────────  "variantGroups": variantGroups[]{    type,    label,    "options": options[]{      label,      priceAdjustment,      isDefault,      inStock,      hexColor    }  }}
...
// Query: *[  _type == "product"  && _id in $ids] {  _id,  name,  "slug": slug.current,  price,  "image": images[0]{ asset->{ _id, url } },  stock,  "variantGroups": variantGroups[]{    type,    "options": options[]{      label,      inStock    }  }}
...
export type REFERRAL_BY_USER_QUERY_RESULT = {
  _id: string;
  clerkUserId: string | null;
  email: string | null;
  name: string | null;
...
export type REFERRAL_CLICKS_BY_CODE_QUERY_RESULT = Array<{
  _id: string;
  code: string | null;
...
export type REFERRAL_LEADERBOARD_QUERY_RESULT = Array<{
  _id: string;
  name: string | null;
  code: string | null;
  conversions: number | null;
