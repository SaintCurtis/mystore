    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-3">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 dark:bg-brand-900/30">
          <ToolIcon className="h-4 w-4 text-brand-600 dark:text-brand-400" />
        </div>
        <div
          className={`flex items-center gap-3 rounded-xl px-4 py-2 text-sm ${
            isComplete
              ? "bg-emerald-50 border border-emerald-200 dark:bg-emerald-950/30 dark:border-emerald-800"
              : "bg-brand-50 border border-brand-200 dark:bg-brand-950/30 dark:border-brand-800"
          }`}
        >
          {isComplete ? (
            <CheckCircle2 className="h-4 w-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
          ) : (
            <Loader2 className="h-4 w-4 text-brand-600 dark:text-brand-400 animate-spin shrink-0" />
          )}
