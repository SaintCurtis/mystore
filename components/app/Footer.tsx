...
const QUICK_LINKS = [
  ...
  { label: "Acasis",                 href: "/?category=acasis" },
  { label: "Custom PCs",             href: "/?category=computers" },
  { label: "EcoFlow",                href: "/?category=ecoflow" },
  { label: "Starlink",               href: "/?category=starlink" },
  { label: "Build My Setup ✨",       href: "/build-my-setup" },
];
        ...
        <div className="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-4 lg:gap-8">
          ...
          <div className="flex flex-col gap-5 lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5 w-fit group">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-500 shadow-sm shadow-brand-500/30 dark:shadow-brand-500/15 transition-all duration-200 group-hover:bg-brand-400">
            ...
            <div className="flex items-center gap-3">
              {SOCIAL_LINKS.map(({ label, href, icon }) => (
                <Link
                  ...
                  aria-label={label}
                  className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-300 dark:border-[#2a2a2a] text-zinc-500 dark:text-[#a3a3a3] transition-all duration-200 hover:border-brand-500/50 hover:bg-brand-500/8 hover:text-brand-600 dark:hover:text-brand-400"
                >
                  {icon}
                </Link>
              ))}
          ...
          <div>
            ...
            <ul className="flex flex-col gap-3">
              {QUICK_LINKS.map(({ label, href }) => (
                <li key={label}>
                  <Link href={href} className="text-sm text-zinc-600 dark:text-[#a3a3a3] transition-colors hover:text-brand-600 dark:hover:text-brand-400">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="flex flex-col gap-4">
              {[
                { href: "mailto:updates@buyfromsaint.com", icon: Mail, label: "Email", value: "updates@buyfromsaint.com", external: false },
                { href: "https://wa.me/2349060898951", icon: Phone, label: "WhatsApp / Call", value: "+234 906 089 8951", external: true },
                { href: "https://t.me/buyfromsaint", icon: Send, label: "Telegram", value: "@buyfromsaint", external: true },
                ...
                <li key={label}>
                  ...
                  >
                    <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-zinc-300 dark:border-[#2a2a2a] bg-white dark:bg-[#111111] text-zinc-500 dark:text-[#a3a3a3] transition-colors group-hover:border-brand-500/40 group-hover:text-brand-600 dark:group-hover:text-brand-400">
                      <Icon className="h-3.5 w-3.5" />
                    </div>
                    <div>
                      <p className="text-xs text-zinc-400 dark:text-[#555]">{label}</p>
                      <p className="text-sm text-zinc-700 dark:text-[#a3a3a3] transition-colors group-hover:text-brand-600 dark:group-hover:text-brand-400 break-all">
                        {value}
                      </p>
                    </div>
                  </Link>
