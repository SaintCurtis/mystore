  ...
  return (
    <div className="flex flex-col h-full">
            ...
            <div>
              ...
              <p className="text-xs leading-tight">
                {ownerActiveRef.current
                  ? <span className="text-brand-500 font-medium">● Live — owner is here</span>
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
        <TagIcon className="w-3.5 h-3.5 text-brand-500 shrink-0" />
        ...
        <div className="px-4 py-3 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0">
          {roundsLeft === 0 ? (
            <p className="text-xs text-center text-zinc-400 py-2">
              Session ended.{" "}
              <button onClick={onClose} className="text-brand-500 underline underline-offset-2">Close</button>
            </p>
          ) : (
            <div className="flex items-end gap-2">
              <textarea
                ...
                className="flex-1 resize-none rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 px-3.5 py-2.5 text-sm text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500 disabled:opacity-50 transition-colors leading-relaxed max-h-28 overflow-y-auto"
                style={{ fieldSizing: "content" } as React.CSSProperties}
              />
              <button
                onClick={sendMessage}
                disabled={!input.trim() || isStreaming}
                className="w-10 h-10 rounded-xl bg-brand-500 hover:bg-brand-400 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shrink-0 transition-all duration-150 shadow-md shadow-brand-500/20"
              >
                {isStreaming
